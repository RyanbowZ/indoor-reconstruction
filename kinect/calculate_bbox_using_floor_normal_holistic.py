...
objectname_list = ["chair"]
objectname = objectname_list[0]
scene_name = "annex"
...
mesh_name = "chair_inline.ply"




mesh_path = f"/ghome/l6/yqliang/littleduck/datasets/{scene_name}/world_corrdinate/" + objectname
preprocess_path = f"/ghome/l6/yqliang/littleduck/datasets/{scene_name}/world_corrdinate/" + objectname +"_preprocess"
projected_path = f"/ghome/l6/yqliang/littleduck/datasets/{scene_name}/world_corrdinate/" + objectname + "_projected"
filter_first_path =f"/ghome/l6/yqliang/littleduck/datasets/{scene_name}/world_corrdinate/" + objectname + "_filtered"
save_path = f"/ghome/l6/yqliang/littleduck/datasets/{scene_name}/world_corrdinate/" + objectname + "_bbox"
...
def project_to_the_floor_and_save():
    ...
    for name in file_list:
        ...
        global floor_normal, on_plane_pt
        on_plane_pts = project_to_the_floor(mesh,floor_normal,on_plane_pt)
        trimesh.PointCloud(on_plane_pts).export(os.path.join(projected_path,name))



def project_to_the_floor(object_mesh, object_normal,object_on_plane_pt,reference = False):


    normal = object_normal / np.linalg.norm(object_normal)


    collected = []
    for v in object_mesh.vertices:
        vec_1 = v - object_on_plane_pt
        distance = np.dot(vec_1, normal)
        projected = vec_1 - distance * normal
        translated_projected = projected + object_on_plane_pt
...
if __name__ == "__main__":
    ...
    cropped_file = objectname + "_inline.ply"
    print(cropped_file + " is processing")
    on_plane_pts = trimesh.load_mesh(os.path.join(pre_filter_path,cropped_file),maintain_order=True,preprocess=False)
    pca, margin_pts = pca_axes(on_plane_pts.vertices)
    ...
    corners, mean, down_spheres = on_plane_bounding_box2(projected_path)
    ...
    on_plane_pts = trimesh.load_mesh(os.path.join(preprocess_path,mesh_name),maintain_order=True,preprocess=False)
    vec = on_plane_pts.vertices - on_plane_pt
    height = np.dot(vec, floor_normal_unit)
    max_val =np.quantile(height, 0.95, method='closest_observation')

    print(mesh_name, "height is " + str(max_val))

    up_corners = corners + max_val * floor_normal_unit
